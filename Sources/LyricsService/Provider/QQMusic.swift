...
// This lyrics result has no karaoke time info.
private let qqSearchBaseURLString2 = "https://u.y.qq.com/cgi-bin/musicu.fcg"
private let qqLyricsBaseURLString2 = "https://c.y.qq.com/lyric/fcgi-bin/fcg_query_lyric_new.fcg"
...
extension LyricsProviders.QQMusic: _LyricsProvider {
    ...
    public func lyricsFetchPublisher(token: LyricsToken) -> AnyPublisher<Lyrics, Never> {
        ...
        return sharedURLSession.cx.dataTaskPublisher(for: req)
            .compactMap { response -> Lyrics? in
                ...
                if let transEncryptedString = try? xmlDocument.nodes(forXPath: "//contentts").first?
                    .stringValue,
    ...
    // MARK: - Another lyrics api, but without karaoke time info.

    public func lyricsSearchPublisher2(request: LyricsSearchRequest) -> AnyPublisher<
        LyricsToken, Never
    > {
        let parameter = [
            "req_1": [
    ...
    public func lyricsFetchPublisher2(token: LyricsToken) -> AnyPublisher<Lyrics, Never> {
        let parameter: [String: Any] = [
            "songmid": token.mid,
            "g_tk": 5381,
        ]
